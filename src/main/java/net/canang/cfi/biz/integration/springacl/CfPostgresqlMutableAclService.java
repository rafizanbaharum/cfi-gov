 ...
 * @since 10/4/13
 */
public class CfPostgresqlMutableAclService extends CfPostgresqlAclService implements MutableAclService {
	...
	public CfPostgresqlMutableAclService(DataSource dataSource, LookupStrategy lookupStrategy, AclCache aclCache) {
	 ...
	 * @param owner
	 *            for the SID column (will be created if there is no acl_sid entry for this
	 *            particular Sid already)
	 */
	protected void createObjectIdentity(ObjectIdentity object, Sid owner) {
		Long sidId = createOrRetrieveSidPrimaryKey(owner, true);
		Long classId = createOrRetrieveClassPrimaryKey(object.getClass(), true);
	 ...
	 *
	 * @param clazz
	 *            to find or create an entry for (this implementation uses the fully-qualified class
	 *            name String)
	 * @param allowCreate
	...
	protected Long createOrRetrieveClassPrimaryKey(Class<?> clazz, boolean allowCreate) {
		List<Long> classIds = jdbcTemplate.queryForList(selectClassPrimaryKey, new Object[] { clazz.getName() },
				Long.class);
		Long classId = null;
	 ...
	 *
	 * @param oid
	 *            to delete the acl_object_identity (and clean up acl_class for that class name if
	 *            appropriate)
	 */
	...
	protected Long retrieveObjectIdentityPrimaryKey(ObjectIdentity oid) {
		try {
			return new Long(jdbcTemplate.queryForLong(selectObjectIdentityPrimaryKey, new Object[] {
